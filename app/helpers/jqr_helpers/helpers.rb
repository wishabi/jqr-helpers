module JqrHelpers::Helpers
  ...
  # Print a tab container. Each option should be a hash of tab title => target.
  # If the target starts with a # it is interpreted as the ID of an element
  # on the page. Otherwise it is interpreted as a URL and the content will
  # be loaded via Ajax.
  # Local tabs need to be defined in the captured block
  # (i.e. the do..end piece).
  # @param options [Hash]
  # @param class_name [String] to go on the surrounding div.
  # @return [String]
  def tab_container(options={}, class_name='', &block)
    content_tag :div, :class => class_name + 'ujs-tab-container' do
      (content_tag :ul do
        s = options.to_a.inject('') do |result, option|
          result << content_tag(:li) do
            link_to option[0], option[1]
          end
        end
        raw s
      end) + raw(capture(&block))
    end
  end
